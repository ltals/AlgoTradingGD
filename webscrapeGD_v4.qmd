...
ScrapeGD <- function(url) {
  ...
  #initiate selenium
  rD <- rsDriver(port = 4570L, browser = "firefox", verbose = FALSE)
  remDr <- rD[["client"]]
  
  #cleaning url to return base
  cleaned_url <- str_remove(url, "\\.htm.*")
  #intitialization url to get review total
  ...
  #extraction of review count
  split <- unlist(strsplit(text_element, " "))
  review_count <- as.numeric(tail(split, 2)[1])
  #num pages returned
...
trustscience <- ScrapeGD("https://www.glassdoor.ca/Reviews/Trust-Science-Reviews-E1341155.htm")
