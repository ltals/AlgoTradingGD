...
trade_func <- function(data = port_y, prop = 2, rate = 2, sent = 2) {
  port_y_norm <- port_y %>% 
  #normalizing indicators
  mutate(mean_rating = mean_rating/5,
         sentiment = sentiment/5) %>% 
  ...
  mutate(logRet = log(port_value/lag(port_value))) %>% 
  drop_na() %>% 
  summarise(sum = sum(logRet)) %>% 
  mutate(prcnt_ret = (exp(sum) - 1)*100) 
...
out <- expand.grid(
  prop = seq(from = 0, to = 1, by = .05),
  rate = seq(from = 0, to = 1, by = .05),
  sent = seq(from = 0, to = 1, by = .05)) %>% 
...
res <- foreach(
  ...
  trade_func(data = port_y, prop = prop, rate = rate, sent = sent)
}
